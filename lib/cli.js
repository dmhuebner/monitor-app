...
cli.responders.help = () => {
  ...
  for (let key in commands) {
    if (commands.hasOwnProperty(key)) {
      ...
      let line = helpers.setColor('yellow', key);
...
cli.responders.stats = () => {
  ...
  for (let key in stats) {
    if (stats.hasOwnProperty(key)) {
      ...
      let line = helpers.setColor('lightblue', key);
      const padding = 50 - line.length;
      ...
      userIds.forEach((userId) => {
        _data.read('users', userId, (error, userData) => {
          if (!error && userData) {
            ...
            let line = `${helpers.setColor('yellow', 'Name:')} ${userData.firstName} ${userData.lastName}, \n`;
            line += `${helpers.setColor('pink', 'Phone:')} ${userData.phone}, \n`;
            line += `${helpers.setColor('lightblue', 'Checks:')} ${numberOfChecks}`;
  ...
  if (userId) {
    ...
    _data.read('users', userId, (error, userData) => {
      if (!error && userData) {
        ...
        console.log(`${helpers.setColor('red', 'Invalid userId')}\nEnter 'list users' to view all existing users.`);
      }
    });
  } else {
    console.log(`${helpers.setColor('red', '--{userId}')} flag is required`);
  }
};
...
cli.init = () => {
  ...
  console.log(helpers.setColor('yellow', 'The CLI is running'));
